...
def test_update_post(fix_create_posts,authorized_client):
    ...
    response=authorized_client.put(f"/posts/{fix_create_posts[1].id}",json=new_data)
    get_post=authorized_client.get(f"/posts/{fix_create_posts[1].id}")

    post_updated=get_post.json()
    
    assert post_updated["Post🪆"]["title"]== new_data["title"]
    assert post_updated["Post🪆"]["content"]== new_data["content"]

    print(f"updated: {post_updated}")
    print(f"status code: {response.status_code}")

def test_unauthorized_update_post(fix_create_posts,client):
     new_data={"title": "unauto ",
        "content":"unauto",
        "published":False}
     response=client.put(f"/posts/{fix_create_posts[3].id}",json=new_data)
     assert  response.status_code==401
     print (f"Response: {response.json()}")


def test_not_owner_update_post(fix_create_posts,authorized_client):
    
        new_data={"title": "not allowes to put ",
        "content":"not allowed to put",
        "published":False}
    

        response=authorized_client.put(f"/posts/{fix_create_posts[3].id}",json=new_data)
        reason=response.json()
        post=authorized_client.get(f"/posts/{fix_create_posts[3].id}")
        get_post=post.json()
        
        assert response.status_code==403
        print (f"Post: {get_post}")
        print (f"reasoon:{reason}")

def test_update_non_existing_post(fix_create_posts,authorized_client):
    ...
    response=authorized_client.put(f"/posts/9999",json=new_data)
