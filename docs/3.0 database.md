...
graph LR
    Client[Client / Postman] -->|1. HTTP Request| API[FastAPI Server]
    API -->|2. Validates Input| Pydantic
    API -->|3. Signs & Verifies Token| JWT
    API -->|4. Builds Queries| SQLAlchemy
    SQLAlchemy -->|5. Translates to SQL| Psycopg2
    Psycopg2 -->|6. Executes| PostgreSQL[(PostgreSQL DB)]
```
...
# 1 DDBB Migration (Alembic) v1.1.13
...
### Query example date and likes with stmt:
``` Python
